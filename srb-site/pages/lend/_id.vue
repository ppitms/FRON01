                ...
                <div class="data" style="width: auto;">
                    <el-form :inline="true" class="demo-form-inline">
                        <el-form-item label="投资金额">
                            <el-input v-model="invest.investAmount" :disabled="lend.status != 1"
...
export default {
    data() {
        return {
            ...
            agree: false, //是否同意协议
            invest: {
                lendId: this.$route.params.id, //标的id
    ...
    watch: {
        "invest.investAmount"(value){
            if(value>=this.lend.amount){
    ...
    methods: {
        ...
        getUserAccountByUserId() {
            this.$axios.$get('/api/core/userAccount/getUserAccountByUserId').then(response => {
        ...
        //投资收益试算(投资金额，年利率，借款期限，还款方式)
        getInterestCount() {
            this.$axios.$get(`/api/core/lendItem/getInterestCount/${this.invest.investAmount}/${this.lend.lendYearRate}/${this.lend.period}/${this.lend.returnMethod}`)
                .then(response => {
        ...
        commitInvest() {
            this.$alert("您即将前往汇付宝投资", "标题", {
                ...
                callback:(action)=>{
                    if(action=='confirm'){
                        this.$axios.$post(`/api/core/lendItem/commitInvest`,this.invest)
                        .then(response=>{
