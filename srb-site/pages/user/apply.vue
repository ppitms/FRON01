<template>
    <div class="personal-main">
        <div class="personal-pay">
            ...
            <div v-if="active === 2">
                <div style="margin-top: 40px">
                    <el-alert v-if="borrowInfoStatus === 2" title="您的借款申请审批已通过" type="success" show-icon
                        :closable="false">
                    </el-alert>

                    <el-alert v-if="borrowInfoStatus === -1" title="您的借款申请审批未通过" type="error" show-icon
                        :closable="false">
                    </el-alert>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        //查询字典下拉列表
        initDictListSelect(){
            let list = ['returnMethod', 'moneyUse'];
            this.$axios.$post('/api/core/dict/getDictMapByDictCodes',list).then(response=>{
                this.returnMethodList = response.data.map.returnMethod;
                this.moneyUseList  = response.data.map.moneyUse;
            })
        },
        //查询借款额度
        getBorrowAmount(){
